...
// Modified to incude restart location
...
// Modified to not break on fault
...
// Modified to wait for fault to correct and then restart
			orientation = FLAT;
			if (config.debug_level > 2) printf("Failed orientation: or:%d, prev:%d, scr:%d, %d\n", orientation, previous_orientation, screen_orientation, FLAT);
			sleep(10);
			goto error_restart;
		}
